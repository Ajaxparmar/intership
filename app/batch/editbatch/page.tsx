...
interface EditForm {
  ...
  // Batch transfer
  batchId:    string;
  // Batch schedule fields (edits the batch itself)
  timingStart: string;
  timingEnd:   string;
  startDate:   string;
  endDate:     string;
...
export default function AdminBookingsPage() {
  ...
  const [editForm,    setEditForm]    = useState<EditForm>({
    fullName: "", whatsappNo: "", email: "", college: "",
    batchId: "", timingStart: "", timingEnd: "", startDate: "", endDate: "",
  });
  const [editErrors,  setEditErrors]  = useState<EditErrors>({});
  const [editSaving,  setEditSaving]  = useState(false);
  const [editError,   setEditError]   = useState("");
  ...
  // Fetch batches for filter dropdown + batch selector
  useEffect(() => {
    (async () => {
      try {
        const res  = await fetch("/api/batch");
  ...
  // ── When batch selector changes, auto-fill schedule fields ──
  const handleBatchChange = (newBatchId: string) => {
    const selected = batches.find(b => b.id === newBatchId);
    setEditForm(p => ({
      ...p,
      batchId: newBatchId,
      timingStart: selected?.timingStart ?? p.timingStart,
      timingEnd:   selected?.timingEnd   ?? p.timingEnd,
      startDate:   selected?.startDate   ?? p.startDate,
      endDate:     selected?.endDate     ?? p.endDate,
    }));
    setEditErrors(p => {
      const n = { ...p };
      delete n.batchId; delete n.timingStart; delete n.timingEnd;
      delete n.startDate; delete n.endDate;
      return n;
    });
  };
  ...
  const openEdit = (b: Booking) => {
    ...
    setEditForm({
      ...
      college:     b.college,
      batchId:     b.batchId,
      timingStart: b.batch.timingStart,
      timingEnd:   b.batch.timingEnd,
  ...
  const validateEdit = (): boolean => {
    ...
    if (!editForm.college)           e.college = "Required";
    if (!editForm.batchId)           e.batchId = "Required";
    if (!editForm.timingStart)       e.timingStart = "Required";
    if (!editForm.timingEnd)         e.timingEnd = "Required";
    if (!editForm.startDate)         e.startDate = "Required";
    if (!editForm.endDate)           e.endDate = "Required";
    if (editForm.timingStart && editForm.timingEnd && editForm.timingStart >= editForm.timingEnd)
      e.timingEnd = "Must be after start";
    if (editForm.startDate && editForm.endDate && editForm.startDate >= editForm.endDate)
      e.endDate = "Must be after start";
    setEditErrors(e);
    return Object.keys(e).length === 0;
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        // Update local state — merge both student + batch fields
        setBookings(prev => prev.map(b => {
          if (b.id !== editTarget.id) return b;
          const newBatch = batches.find(bt => bt.id === editForm.batchId) ?? b.batch;
          return {
            ...b,
            ...
            batch: {
              ...newBatch,
              timingStart: editForm.timingStart,
              timingEnd:   editForm.timingEnd,
  ...
  // ── Derived: is batch being changed? ─────────────────
  const isBatchChanged = editTarget && editForm.batchId !== editTarget.batchId;
          ...
          <div className="fixed inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto">
            ...
            <motion.div
              ...
              <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
                ...
                {/* ── Batch Transfer ── */}
                <SectionDivider label="Batch Assignment" />

                <EField label="Batch" error={editErrors.batchId}>
                  <div className="relative">
                    <ArrowLeftRight size={15} className={cn("absolute left-3.5 top-1/2 -translate-y-1/2 z-10",
                      editErrors.batchId ? "text-red-400" : "text-slate-400")} />
                    <select
                ...
                {/* Batch transfer notice */}
                <AnimatePresence>
                  {isBatchChanged && selectedBatch && (
                    <motion.div
                      initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                      className="flex items-start gap-3 p-3.5 bg-amber-50 border border-amber-200 rounded-2xl">
                      <ArrowLeftRight size={15} className="text-amber-500 shrink-0 mt-0.5" />
                      <p className="text-xs text-amber-700">
                        Student will be moved from <strong>{editTarget.batch.name}</strong> to{" "}
                        <strong>{selectedBatch.name}</strong>. Seat counts will not be adjusted.
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                {/* ── Batch Schedule ── */}
                <SectionDivider label="Batch Schedule" />

                {/* Timing */}
                <div className="grid grid-cols-2 gap-4">
                  <EField label="Start Time" error={editErrors.timingStart}>
                    <div className="relative">
                      <Clock size={15} className={cn("absolute left-3.5 top-1/2 -translate-y-1/2 z-10",
                        editErrors.timingStart ? "text-red-400" : "text-slate-400")} />
                      <input
                        type="time"
                        value={editForm.timingStart}
                        onChange={e => updEdit("timingStart", e.target.value)}
                        className={cn(
                          "w-full pl-10 pr-4 py-3 border rounded-xl text-sm bg-slate-50 outline-none transition-all focus:bg-white focus:ring-4",
                          editErrors.timingStart
                            ? "border-red-400 focus:ring-red-100"
                            : "border-slate-200 focus:border-blue-500 focus:ring-blue-100"
                        )}
                      />
                  ...
                  <EField label="End Time" error={editErrors.timingEnd}>
                    <div className="relative">
                      <Clock size={15} className={cn("absolute left-3.5 top-1/2 -translate-y-1/2 z-10",
                        editErrors.timingEnd ? "text-red-400" : "text-slate-400")} />
                      <input
                        type="time"
                        value={editForm.timingEnd}
                        onChange={e => updEdit("timingEnd", e.target.value)}
                        className={cn(
                          "w-full pl-10 pr-4 py-3 border rounded-xl text-sm bg-slate-50 outline-none transition-all focus:bg-white focus:ring-4",
                          editErrors.timingEnd
                            ? "border-red-400 focus:ring-red-100"
                            : "border-slate-200 focus:border-blue-500 focus:ring-blue-100"
                        )}
                      />
                ...
                {/* Dates */}
                <div className="grid grid-cols-2 gap-4">
                  <EField label="Start Date" error={editErrors.startDate}>
                    <div className="relative">
                      <Calendar size={15} className={cn("absolute left-3.5 top-1/2 -translate-y-1/2 z-10",
                        editErrors.startDate ? "text-red-400" : "text-slate-400")} />
                      <input
                        type="date"
                  ...
                  <EField label="End Date" error={editErrors.endDate}>
                    <div className="relative">
                      <Calendar size={15} className={cn("absolute left-3.5 top-1/2 -translate-y-1/2 z-10",
                        editErrors.endDate ? "text-red-400" : "text-slate-400")} />
                      <input
                        type="date"
        ...
        {deleteTarget && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              <h3 className="text-xl font-bold text-slate-800 mb-1">Delete Booking?</h3>
              <p className="text-sm text-slate-400 mb-1">This will permanently remove the booking for</p>
              <p className="font-bold text-slate-700 mb-1">{deleteTarget.fullName}</p>
              <p className="text-xs text-slate-400 mb-6">{deleteTarget.batch.name} · {deleteTarget.batch.course}</p>
              <div className="flex gap-3">
                <button disabled={deleting} onClick={() => setDeleteTarget(null)}
