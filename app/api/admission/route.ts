...
export async function POST(request: Request) {
  try {
    ...
    // Check for duplicate: same rollNo + college + academicClass
    const existing = await prisma.admission.findFirst({
      where: { rollNo: data.rollNo, college: data.college, academicClass: data.academicClass },
    });
    if (existing) {
      return NextResponse.json(
        { error: `Roll No. ${data.rollNo} is already registered for ${data.academicClass} at the selected college. Each student can only apply once.` },
        { status: 409 }
      );
    }
